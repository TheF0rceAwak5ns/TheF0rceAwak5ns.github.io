...
Ok if we had the password we could have passed root easily but this is not the case... so we will focus on the second right sudo. So we can run the [monitor.sh](http://monitor.sh/) script with root rights without the jack password try to understand the script!
...
CSRF : https://www.thehacker.recipes/web/inputs/crlf-injection
